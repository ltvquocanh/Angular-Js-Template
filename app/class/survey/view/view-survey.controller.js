'use strict';
angular.module('mainApp').controller('ViewSurey',
    ['$scope', '$http', '$window', 'cfpLoadingBar', '$stateParams', 'ModalService', 'AuthService', 'USER_ROLES',
        function ($scope, $http, $window, cfpLoadingBar, $stateParams, ModalService, AuthService, USER_ROLES) {
            ...
            function getSurvey(surveyId) {
                $http.get("Survey/GetSurveyForViewSurvey", {
            ...
            //check survey Viewer or Complete
            function isDisabled() {
                for (var i = 0; i < $scope.survey.Users.length; i++) {
                    if (($scope.currentUser.Id == $scope.survey.Users[i].ParticipantId && $scope.survey.Users[i].Status == 2)
                        || $scope.survey.Status == 3) {
                        $scope.isDisabled = true;
                    }
                    else if ($scope.currentUser.Id == $scope.survey.Users[i].ParticipantId && $scope.survey.Users[i].Status == 1) {
                        $scope.isDisabled = false;
                    }
                }
            }
